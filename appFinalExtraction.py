    ...
    for filename in os.listdir(dir):
        if i >= 18:
            for filename2 in os.listdir(dir+filename):
                print(filename ," :" ,filename2 , " i = ",i)
                # cap = cv.VideoCapture(dir+filename+'/'+filename2)
                main1(dir+filename+'/'+filename2, i)
        i+=1
        # elif filename == 'Address':
        #     for filename2 in os.listdir(dir+filename):
        #         print(filename ," :" ,filename2)
        #         # cap = cv.VideoCapture(dir+filename+'/'+filename2)
        #         main1(dir+filename+'/'+filename2, 1)
        

    return 0
def main1(link, number1):
    # Argument parsing #################################################################
    args = get_args()
    ...
    # cap = cv.VideoCapture('/media/samyak/DATA/DATA/2 DO/SAMYAK/HandMotion/Dataset/Address/C0567.MP4')
    # cap = cv.VideoCapture('/media/samyak/DATA/DATA/2 DO/SAMYAK/HandMotion/Dataset/Account/C0194.MP4')
    cap = cv.VideoCapture(link)
    number, mode = number1,1
    ...
    with open('model/keypoint_classifier/keypoint_classifier_label.csv',
        ...
        # print("keypoint_classifier_labels: ",keypoint_classifier_labels[0])
    with open(
...
def logging_csv(number, mode, landmark_list, point_history_list):
    ...
    if mode == 1 :
        try:
            csv_path = 'model/keypoint_classifier/keypointF.csv'
            # if csv is not exist, create csv   
            if not os.path.exists(csv_path):
                with open(csv_path, 'w', newline="") as f:
                    writer = csv.writer(f)
                    writer.writerow(['label', *list(range(0, 63))])
            with open(csv_path, 'a', newline="") as f:
                writer = csv.writer(f)
                writer.writerow([number, *landmark_list])
        except:
            print("csv logging error")

    if mode == 2 and (0 <= number <= 9):
        csv_path = 'model/point_history_classifier/point_historyF.csv'
        with open(csv_path, 'a', newline="") as f:
            writer = csv.writer(f)
