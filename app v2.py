...
def main():
    ...
    cap = cv.VideoCapture('/media/samyak/DATA/DATA/2 DO/SAMYAK/HandMotion/Dataset/Account/C0261.MP4')
    ...
    while True:
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                ...
                debug_image = draw_landmarks2(debug_image, landmark_list)
...
def draw_landmarks2(image, landmark_point):
    ...
    return image
def draw_landmarks1(image, landmark_point):
    if len(landmark_point) > 0:
        def draw_lines(points, color1, color2):
            for i in range(0, len(points)-1, 2):
                cv.line(image, tuple(points[i]), tuple(points[i+1]), color1, 6)
